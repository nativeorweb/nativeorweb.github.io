...
public class Sieve implements Iterator<Long> {
    ...
    public static long DEFAULT_NUMBER = 80000000;

    public static void execute(long numberN){

        long n = numberN;
        Iterator<Long> gen = new Sieve();
        int count = 0;
        for (long i = 1; i < n; i++, gen.next());
        long prime = gen.next();
        while (gen.next() <= n) count++;
        //    Log.v("info","Found " + prime + " primes up to " + n + " in " + 0 + " milliseconds.");
    }
        ...
        if (this.bi < 1) {
            ...
            if (this.lowi <= 0) { // special culling for first page as no base primes yet:
                ...
                if (this.bpa.isEmpty()) { // if this is the first page after the zero one:
                    this.bps = new Sieve(); // initialize separate base primes stream:
